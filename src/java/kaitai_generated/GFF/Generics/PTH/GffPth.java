...
/**
 * **PTH** resources are **GFF3** on disk (Aurora `GFF ` prefix + V3.x version). Wire layout is fully defined by
 ...
 * 
 * FileType / restype id **3003** — see `bioware_type_ids::xoreos_file_type_id` enum member `pth`.
 ...
 */
public class GffPth extends KaitaiStruct {
    public static GffPth fromFile(String fileName) throws IOException {
        return new GffPth(new ByteBufferKaitaiStream(fileName));
    }

    public GffPth(KaitaiStream _io) {
        this(_io, null, null);
    }

    public GffPth(KaitaiStream _io, KaitaiStruct _parent) {
        this(_io, _parent, null);
    }

    public GffPth(KaitaiStream _io, KaitaiStruct _parent, GffPth _root) {
        super(_io);
    ...
    private GffPth _root;
    private KaitaiStruct _parent;

    /**
     * Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per PTH template docs / PyKotor `gff_auto`.
     */
    public Gff.GffUnionFile contents() { return contents; }
    public GffPth _root() { return _root; }
