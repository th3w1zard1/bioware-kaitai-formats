...
/**
 * **UTP** resources are **GFF3** on disk (Aurora `GFF ` prefix + V3.x version). Wire layout is fully defined by
 ...
 * 
 * FileType / restype id **2044** — see `bioware_type_ids::xoreos_file_type_id` enum member `utp`.
 ...
 */
public class GffUtp extends KaitaiStruct {
    public static GffUtp fromFile(String fileName) throws IOException {
        return new GffUtp(new ByteBufferKaitaiStream(fileName));
    }

    public GffUtp(KaitaiStream _io) {
        this(_io, null, null);
    }

    public GffUtp(KaitaiStream _io, KaitaiStruct _parent) {
        this(_io, _parent, null);
    }

    public GffUtp(KaitaiStream _io, KaitaiStruct _parent, GffUtp _root) {
        super(_io);
    ...
    private GffUtp _root;
    private KaitaiStruct _parent;

    /**
     * Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per UTP template docs / PyKotor `gff_auto`.
     */
    public Gff.GffUnionFile contents() { return contents; }
    public GffUtp _root() { return _root; }
