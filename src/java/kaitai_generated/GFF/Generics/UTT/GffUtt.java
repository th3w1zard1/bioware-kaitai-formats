...
/**
 * **UTT** resources are **GFF3** on disk (Aurora `GFF ` prefix + V3.x version). Wire layout is fully defined by
 ...
 * 
 * FileType / restype id **2032** — see `bioware_type_ids::xoreos_file_type_id` enum member `utt`.
 ...
 */
public class GffUtt extends KaitaiStruct {
    public static GffUtt fromFile(String fileName) throws IOException {
        return new GffUtt(new ByteBufferKaitaiStream(fileName));
    }

    public GffUtt(KaitaiStream _io) {
        this(_io, null, null);
    }

    public GffUtt(KaitaiStream _io, KaitaiStruct _parent) {
        this(_io, _parent, null);
    }

    public GffUtt(KaitaiStream _io, KaitaiStruct _parent, GffUtt _root) {
        super(_io);
    ...
    private GffUtt _root;
    private KaitaiStruct _parent;

    /**
     * Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per UTT template docs / PyKotor `gff_auto`.
     */
    public Gff.GffUnionFile contents() { return contents; }
    public GffUtt _root() { return _root; }
