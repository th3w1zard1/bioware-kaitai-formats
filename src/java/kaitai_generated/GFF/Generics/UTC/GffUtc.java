...
/**
 * **UTC** resources are **GFF3** on disk (Aurora `GFF ` prefix + V3.x version). Wire layout is fully defined by
 ...
 * 
 * FileType / restype id **2027** — see `bioware_type_ids::xoreos_file_type_id` enum member `utc`.
 ...
 */
public class GffUtc extends KaitaiStruct {
    public static GffUtc fromFile(String fileName) throws IOException {
        return new GffUtc(new ByteBufferKaitaiStream(fileName));
    }

    public GffUtc(KaitaiStream _io) {
        this(_io, null, null);
    }

    public GffUtc(KaitaiStream _io, KaitaiStruct _parent) {
        this(_io, _parent, null);
    }

    public GffUtc(KaitaiStream _io, KaitaiStruct _parent, GffUtc _root) {
        super(_io);
    ...
    private GffUtc _root;
    private KaitaiStruct _parent;

    /**
     * Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per UTC template docs / PyKotor `gff_auto`.
     */
    public Gff.GffUnionFile contents() { return contents; }
    public GffUtc _root() { return _root; }
