...
/**
 * **SAV** resources are **GFF3** on disk (Aurora `GFF ` prefix + V3.x version). Wire layout is fully defined by
 ...
 * 
 * FileType / restype id **2057** — see `bioware_type_ids::xoreos_file_type_id` enum member `sav`.
 ...
 */
public class GffSav extends KaitaiStruct {
    public static GffSav fromFile(String fileName) throws IOException {
        return new GffSav(new ByteBufferKaitaiStream(fileName));
    }

    public GffSav(KaitaiStream _io) {
        this(_io, null, null);
    }

    public GffSav(KaitaiStream _io, KaitaiStruct _parent) {
        this(_io, _parent, null);
    }

    public GffSav(KaitaiStream _io, KaitaiStruct _parent, GffSav _root) {
        super(_io);
    ...
    private GffSav _root;
    private KaitaiStruct _parent;

    /**
     * Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per SAV template docs / PyKotor `gff_auto`.
     */
    public Gff.GffUnionFile contents() { return contents; }
    public GffSav _root() { return _root; }
