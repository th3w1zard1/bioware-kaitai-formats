...
/**
 * **UTD** resources are **GFF3** on disk (Aurora `GFF ` prefix + V3.x version). Wire layout is fully defined by
 ...
 * 
 * FileType / restype id **2042** — see `bioware_type_ids::xoreos_file_type_id` enum member `utd`.
 ...
 */
public class GffUtd extends KaitaiStruct {
    public static GffUtd fromFile(String fileName) throws IOException {
        return new GffUtd(new ByteBufferKaitaiStream(fileName));
    }

    public GffUtd(KaitaiStream _io) {
        this(_io, null, null);
    }

    public GffUtd(KaitaiStream _io, KaitaiStruct _parent) {
        this(_io, _parent, null);
    }

    public GffUtd(KaitaiStream _io, KaitaiStruct _parent, GffUtd _root) {
        super(_io);
    ...
    private GffUtd _root;
    private KaitaiStruct _parent;

    /**
     * Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per UTD template docs / PyKotor `gff_auto`.
     */
    public Gff.GffUnionFile contents() { return contents; }
    public GffUtd _root() { return _root; }
