...
/**
 * **GUI** resources are **GFF3** on disk (Aurora `GFF ` prefix + V3.x version). Wire layout is fully defined by
 ...
 * 
 * FileType / restype id **2047** — see `bioware_type_ids::xoreos_file_type_id` enum member `gui`.
 ...
 */
public class GffGui extends KaitaiStruct {
    public static GffGui fromFile(String fileName) throws IOException {
        return new GffGui(new ByteBufferKaitaiStream(fileName));
    }

    public GffGui(KaitaiStream _io) {
        this(_io, null, null);
    }

    public GffGui(KaitaiStream _io, KaitaiStruct _parent) {
        this(_io, _parent, null);
    }

    public GffGui(KaitaiStream _io, KaitaiStruct _parent, GffGui _root) {
        super(_io);
    ...
    private GffGui _root;
    private KaitaiStruct _parent;

    /**
     * Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per GUI template docs / PyKotor `gff_auto`.
     */
    public Gff.GffUnionFile contents() { return contents; }
    public GffGui _root() { return _root; }
