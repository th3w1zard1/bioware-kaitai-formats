...
/**
 * **UTE** resources are **GFF3** on disk (Aurora `GFF ` prefix + V3.x version). Wire layout is fully defined by
 ...
 * 
 * FileType / restype id **2040** — see `bioware_type_ids::xoreos_file_type_id` enum member `ute`.
 ...
 */
public class GffUte extends KaitaiStruct {
    public static GffUte fromFile(String fileName) throws IOException {
        return new GffUte(new ByteBufferKaitaiStream(fileName));
    }

    public GffUte(KaitaiStream _io) {
        this(_io, null, null);
    }

    public GffUte(KaitaiStream _io, KaitaiStruct _parent) {
        this(_io, _parent, null);
    }

    public GffUte(KaitaiStream _io, KaitaiStruct _parent, GffUte _root) {
        super(_io);
    ...
    private GffUte _root;
    private KaitaiStruct _parent;

    /**
     * Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per UTE template docs / PyKotor `gff_auto`.
     */
    public Gff.GffUnionFile contents() { return contents; }
    public GffUte _root() { return _root; }
