...
/**
 * **BTT** resources are **GFF3** on disk (Aurora `GFF ` prefix + V3.x version). Wire layout is fully defined by
 ...
 * 
 * FileType / restype id **2031** — see `bioware_type_ids::xoreos_file_type_id` enum member `btt`.
 ...
 */
public class GffBtt extends KaitaiStruct {
    public static GffBtt fromFile(String fileName) throws IOException {
        return new GffBtt(new ByteBufferKaitaiStream(fileName));
    }

    public GffBtt(KaitaiStream _io) {
        this(_io, null, null);
    }

    public GffBtt(KaitaiStream _io, KaitaiStruct _parent) {
        this(_io, _parent, null);
    }

    public GffBtt(KaitaiStream _io, KaitaiStruct _parent, GffBtt _root) {
        super(_io);
    ...
    private GffBtt _root;
    private KaitaiStruct _parent;

    /**
     * Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per BTT template docs / PyKotor `gff_auto`.
     */
    public Gff.GffUnionFile contents() { return contents; }
    public GffBtt _root() { return _root; }
