...
/**
 * **GIC** resources are **GFF3** on disk (Aurora `GFF ` prefix + V3.x version). Wire layout is fully defined by
 ...
 * 
 * FileType / restype id **2046** — see `bioware_type_ids::xoreos_file_type_id` enum member `gic`.
 ...
 */
public class GffGic extends KaitaiStruct {
    public static GffGic fromFile(String fileName) throws IOException {
        return new GffGic(new ByteBufferKaitaiStream(fileName));
    }

    public GffGic(KaitaiStream _io) {
        this(_io, null, null);
    }

    public GffGic(KaitaiStream _io, KaitaiStruct _parent) {
        this(_io, _parent, null);
    }

    public GffGic(KaitaiStream _io, KaitaiStruct _parent, GffGic _root) {
        super(_io);
    ...
    private GffGic _root;
    private KaitaiStruct _parent;

    /**
     * Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per GIC template docs / PyKotor `gff_auto`.
     */
    public Gff.GffUnionFile contents() { return contents; }
    public GffGic _root() { return _root; }
