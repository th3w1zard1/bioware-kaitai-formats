...
/**
 * **QST** resources are **GFF3** on disk (Aurora `GFF ` prefix + V3.x version). Wire layout is fully defined by
 ...
 * 
 * FileType / restype id **2084** — see `bioware_type_ids::xoreos_file_type_id` enum member `qst`.
 ...
 */
public class GffQst extends KaitaiStruct {
    public static GffQst fromFile(String fileName) throws IOException {
        return new GffQst(new ByteBufferKaitaiStream(fileName));
    }

    public GffQst(KaitaiStream _io) {
        this(_io, null, null);
    }

    public GffQst(KaitaiStream _io, KaitaiStruct _parent) {
        this(_io, _parent, null);
    }

    public GffQst(KaitaiStream _io, KaitaiStruct _parent, GffQst _root) {
        super(_io);
    ...
    private GffQst _root;
    private KaitaiStruct _parent;

    /**
     * Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per QST template docs / PyKotor `gff_auto`.
     */
    public Gff.GffUnionFile contents() { return contents; }
    public GffQst _root() { return _root; }
