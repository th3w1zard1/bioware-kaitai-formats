...
class Mdl(KaitaiStruct):
    ...
    def __init__(
        self,
        _io: KaitaiStream,
        _parent: KaitaiStruct | None = None,
        _root: KaitaiStruct | None = None,
    ) -> None:
        super(Mdl, self).__init__(_io)
        self._io: KaitaiStream
        self._parent: KaitaiStruct | None = _parent
        self._root: KaitaiStruct | None = _root or self
        self._read()

    def _read(self):
        self.file_header: Mdl.FileHeader = Mdl.FileHeader(self._io, self, self._root)
        self.model_header: Mdl.ModelHeader = Mdl.ModelHeader(self._io, self, self._root)
    ...
    class AnimationEvent(KaitaiStruct):
        ...
        def _read(self):
            self.activation_time: float = self._io.read_f4le()
            self.event_name: str = (KaitaiStream.bytes_terminate(self._io.read_bytes(32), 0, False)).decode("ASCII")
    ...
    class AnimationHeader(KaitaiStruct):
        ...
        def _read(self):
            self.geo_header: Mdl.GeometryHeader = Mdl.GeometryHeader(self._io, self, self._root)
            self.animation_length: float = self._io.read_f4le()
            self.transition_time: float = self._io.read_f4le()
            self.animation_root: str = (KaitaiStream.bytes_terminate(self._io.read_bytes(32), 0, False)).decode("ASCII")
            self.event_array_offset: int = self._io.read_u4le()
            self.event_count: int = self._io.read_u4le()
            self.event_count_duplicate: int = self._io.read_u4le()
            self.unknown: int = self._io.read_u4le()
    ...
    class AnimmeshHeader(KaitaiStruct):
        ...
        def _read(self):
            self.trimesh_base: Mdl.TrimeshHeader = Mdl.TrimeshHeader(self._io, self, self._root)
            self.unknown: float = self._io.read_f4le()
            self.unknown_array: Mdl.ArrayDefinition = Mdl.ArrayDefinition(self._io, self, self._root)
            self.unknown_floats: list[float] = []
            for i in range(9):
                self.unknown_floats.append(float(self._io.read_f4le()))
    ...
    class ArrayDefinition(KaitaiStruct):
        ...
        def _read(self):
            self.offset: int = self._io.read_s4le()
            self.count: int = self._io.read_u4le()
            self.count_duplicate: int = self._io.read_u4le()
    ...
    class Controller(KaitaiStruct):
        ...
        def _read(self):
            self.type: bioware_mdl_common.BiowareMdlCommon.ControllerType = KaitaiStream.resolve_enum(bioware_mdl_common.BiowareMdlCommon.ControllerType, self._io.read_u4le())
            self.unknown: int = self._io.read_u2le()
            self.row_count: int = self._io.read_u2le()
            self.time_index: int = self._io.read_u2le()
            self.data_index: int = self._io.read_u2le()
            self.column_count: int = self._io.read_u1()
            self.padding: list[int] = []
            for i in range(3):
                self.padding.append(int(self._io.read_u1()))
    ...
    class DanglymeshHeader(KaitaiStruct):
        ...
        def _read(self):
            self.trimesh_base: Mdl.TrimeshHeader = Mdl.TrimeshHeader(self._io, self, self._root)
            self.constraints_offset: int = self._io.read_u4le()
            self.constraints_count: int = self._io.read_u4le()
            self.constraints_count_duplicate: int = self._io.read_u4le()
            self.displacement: float = self._io.read_f4le()
            self.tightness: float = self._io.read_f4le()
            self.period: float = self._io.read_f4le()
            self.unknown: int = self._io.read_u4le()
    ...
    class EmitterHeader(KaitaiStruct):
        ...
        def _read(self):
            self.dead_space: float = self._io.read_f4le()
            self.blast_radius: float = self._io.read_f4le()
            self.blast_length: float = self._io.read_f4le()
            self.branch_count: int = self._io.read_u4le()
            self.control_point_smoothing: float = self._io.read_f4le()
            self.x_grid: int = self._io.read_u4le()
            self.y_grid: int = self._io.read_u4le()
            self.padding_unknown: int = self._io.read_u4le()
            self.update_script: str = (KaitaiStream.bytes_terminate(self._io.read_bytes(32), 0, False)).decode("ASCII")
            self.render_script: str = (KaitaiStream.bytes_terminate(self._io.read_bytes(32), 0, False)).decode("ASCII")
            self.blend_script: str = (KaitaiStream.bytes_terminate(self._io.read_bytes(32), 0, False)).decode("ASCII")
            self.texture_name: str = (KaitaiStream.bytes_terminate(self._io.read_bytes(32), 0, False)).decode("ASCII")
            self.chunk_name: str = (KaitaiStream.bytes_terminate(self._io.read_bytes(32), 0, False)).decode("ASCII")
            self.two_sided_texture: int = self._io.read_u4le()
            self.loop: int = self._io.read_u4le()
            self.render_order: int = self._io.read_u2le()
            self.frame_blending: int = self._io.read_u1()
            self.depth_texture_name: str = (KaitaiStream.bytes_terminate(self._io.read_bytes(32), 0, False)).decode("ASCII")
            self.padding: int = self._io.read_u1()
            self.flags: int = self._io.read_u4le()
    ...
    class FileHeader(KaitaiStruct):
        ...
        def _read(self):
            self.unused: int = self._io.read_u4le()
            self.mdl_size: int = self._io.read_u4le()
            self.mdx_size: int = self._io.read_u4le()
    ...
    class GeometryHeader(KaitaiStruct):
        ...
        def _read(self):
            self.function_pointer_0: int = self._io.read_u4le()
            self.function_pointer_1: int = self._io.read_u4le()
            self.model_name: str = (KaitaiStream.bytes_terminate(self._io.read_bytes(32), 0, False)).decode("ASCII")
            self.root_node_offset: int = self._io.read_u4le()
            self.node_count: int = self._io.read_u4le()
            self.unknown_array_1: Mdl.ArrayDefinition = Mdl.ArrayDefinition(self._io, self, self._root)
            self.unknown_array_2: Mdl.ArrayDefinition = Mdl.ArrayDefinition(self._io, self, self._root)
            self.reference_count: int = self._io.read_u4le()
            self.geometry_type: int = self._io.read_u1()
            self.padding: list[int] = []
            for i in range(3):
                self.padding.append(int(self._io.read_u1()))
    ...
    class LightHeader(KaitaiStruct):
        ...
        def _read(self):
            self.unknown: list[float] = []
            for i in range(4):
                self.unknown.append(float(self._io.read_f4le()))

            self.flare_sizes_offset: int = self._io.read_u4le()
            self.flare_sizes_count: int = self._io.read_u4le()
            self.flare_sizes_count_duplicate: int = self._io.read_u4le()
            self.flare_positions_offset: int = self._io.read_u4le()
            self.flare_positions_count: int = self._io.read_u4le()
            self.flare_positions_count_duplicate: int = self._io.read_u4le()
            self.flare_color_shifts_offset: int = self._io.read_u4le()
            self.flare_color_shifts_count: int = self._io.read_u4le()
            self.flare_color_shifts_count_duplicate: int = self._io.read_u4le()
            self.flare_texture_names_offset: int = self._io.read_u4le()
            self.flare_texture_names_count: int = self._io.read_u4le()
            self.flare_texture_names_count_duplicate: int = self._io.read_u4le()
            self.flare_radius: float = self._io.read_f4le()
            self.light_priority: int = self._io.read_u4le()
            self.ambient_only: int = self._io.read_u4le()
            self.dynamic_type: int = self._io.read_u4le()
            self.affect_dynamic: int = self._io.read_u4le()
            self.shadow: int = self._io.read_u4le()
            self.flare: int = self._io.read_u4le()
            self.fading_light: int = self._io.read_u4le()
    ...
    class LightsaberHeader(KaitaiStruct):
        ...
        def _read(self):
            self.trimesh_base: Mdl.TrimeshHeader = Mdl.TrimeshHeader(self._io, self, self._root)
            self.vertices_offset: int = self._io.read_u4le()
            self.texcoords_offset: int = self._io.read_u4le()
            self.normals_offset: int = self._io.read_u4le()
            self.unknown1: int = self._io.read_u4le()
            self.unknown2: int = self._io.read_u4le()
    ...
    class MdlAnimationEntry(KaitaiStruct):
        ...
        def __init__(self, anim_index: int, _io: KaitaiStream, _parent: KaitaiStruct | None = None, _root: KaitaiStruct | None = None):
            super(Mdl.MdlAnimationEntry, self).__init__(_io)
            self._io: KaitaiStream
            self._parent: KaitaiStruct | None = _parent
            self._root: KaitaiStruct | None = _root or self
            self.anim_index: int = anim_index
            self._read()
    ...
    class ModelHeader(KaitaiStruct):
        ...
        def _read(self):
            self.geometry: Mdl.GeometryHeader = Mdl.GeometryHeader(self._io, self, self._root)
            self.model_type: bioware_mdl_common.BiowareMdlCommon.ModelClassification = KaitaiStream.resolve_enum(bioware_mdl_common.BiowareMdlCommon.ModelClassification, self._io.read_u1())
            self.unknown0: int = self._io.read_u1()
            self.padding0: int = self._io.read_u1()
            self.fog: int = self._io.read_u1()
            self.unknown1: int = self._io.read_u4le()
            self.offset_to_animations: int = self._io.read_u4le()
            self.animation_count: int = self._io.read_u4le()
            self.animation_count2: int = self._io.read_u4le()
            self.unknown2: int = self._io.read_u4le()
            self.bounding_box_min: Mdl.Vec3f = Mdl.Vec3f(self._io, self, self._root)
            self.bounding_box_max: Mdl.Vec3f = Mdl.Vec3f(self._io, self, self._root)
            self.radius: float = self._io.read_f4le()
            self.animation_scale: float = self._io.read_f4le()
            self.supermodel_name: str = (KaitaiStream.bytes_terminate(self._io.read_bytes(32), 0, False)).decode("ASCII")
            self.offset_to_super_root: int = self._io.read_u4le()
            self.unknown3: int = self._io.read_u4le()
            self.mdx_data_size: int = self._io.read_u4le()
            self.mdx_data_offset: int = self._io.read_u4le()
            self.offset_to_name_offsets: int = self._io.read_u4le()
            self.name_offsets_count: int = self._io.read_u4le()
            self.name_offsets_count2: int = self._io.read_u4le()
    ...
    class Node(KaitaiStruct):
        ...
        def _read(self):
            self.header: Mdl.NodeHeader = Mdl.NodeHeader(self._io, self, self._root)
            if self.header.node_type == 3:
                pass
                self.light_sub_header: Mdl.LightHeader = Mdl.LightHeader(self._io, self, self._root)

            if self.header.node_type == 5:
                pass
                self.emitter_sub_header: Mdl.EmitterHeader = Mdl.EmitterHeader(self._io, self, self._root)

            if self.header.node_type == 17:
                pass
                self.reference_sub_header: Mdl.ReferenceHeader = Mdl.ReferenceHeader(self._io, self, self._root)

            if self.header.node_type == 33:
                pass
                self.trimesh_sub_header: Mdl.TrimeshHeader = Mdl.TrimeshHeader(self._io, self, self._root)

            if self.header.node_type == 97:
                pass
                self.skinmesh_sub_header: Mdl.SkinmeshHeader = Mdl.SkinmeshHeader(self._io, self, self._root)

            if self.header.node_type == 161:
                pass
                self.animmesh_sub_header: Mdl.AnimmeshHeader = Mdl.AnimmeshHeader(self._io, self, self._root)

            if self.header.node_type == 289:
                pass
                self.danglymesh_sub_header: Mdl.DanglymeshHeader = Mdl.DanglymeshHeader(self._io, self, self._root)

            if self.header.node_type == 545:
                pass
                self.aabb_sub_header: Mdl.AabbHeader = Mdl.AabbHeader(self._io, self, self._root)

            if self.header.node_type == 2081:
                pass
                self.lightsaber_sub_header: Mdl.LightsaberHeader = Mdl.LightsaberHeader(self._io, self, self._root)
    ...
    class NodeHeader(KaitaiStruct):
        ...
        def _read(self):
            self.node_type: int = self._io.read_u2le()
            self.node_index: int = self._io.read_u2le()
            self.node_name_index: int = self._io.read_u2le()
            self.padding: int = self._io.read_u2le()
            self.root_node_offset: int = self._io.read_u4le()
            self.parent_node_offset: int = self._io.read_u4le()
            self.position: Mdl.Vec3f = Mdl.Vec3f(self._io, self, self._root)
            self.orientation: Mdl.Quaternion = Mdl.Quaternion(self._io, self, self._root)
            self.child_array_offset: int = self._io.read_u4le()
            self.child_count: int = self._io.read_u4le()
            self.child_count_duplicate: int = self._io.read_u4le()
            self.controller_array_offset: int = self._io.read_u4le()
            self.controller_count: int = self._io.read_u4le()
            self.controller_count_duplicate: int = self._io.read_u4le()
            self.controller_data_offset: int = self._io.read_u4le()
            self.controller_data_count: int = self._io.read_u4le()
            self.controller_data_count_duplicate: int = self._io.read_u4le()
    ...
    class Quaternion(KaitaiStruct):
        ...
        def _read(self):
            self.w: float = self._io.read_f4le()
            self.x: float = self._io.read_f4le()
            self.y: float = self._io.read_f4le()
            self.z: float = self._io.read_f4le()

        def _fetch_instances(self):
            pass

    class ReferenceHeader(KaitaiStruct):
        """Reference header 36 (0x24) bytes."""
        ...
        def _read(self):
            self.model_resref: str = (KaitaiStream.bytes_terminate(self._io.read_bytes(32), 0, False)).decode("ASCII")
            self.reattachable: int = self._io.read_u4le()
    ...
    class SkinmeshHeader(KaitaiStruct):
        ...
        def _read(self):
            self.trimesh_base: Mdl.TrimeshHeader = Mdl.TrimeshHeader(self._io, self, self._root)
            self.unknown_weights: int = self._io.read_s4le()
            self.padding1: list[int] = []
            for i in range(8):
                self.padding1.append(self._io.read_u1())

            self.mdx_bone_weights_offset: int = self._io.read_u4le()
            self.mdx_bone_indices_offset: int = self._io.read_u4le()
            self.bone_map_offset: int = self._io.read_u4le()
            self.bone_map_count: int = self._io.read_u4le()
            self.qbones_offset: int = self._io.read_u4le()
            self.qbones_count: int = self._io.read_u4le()
            self.qbones_count_duplicate: int = self._io.read_u4le()
            self.tbones_offset: int = self._io.read_u4le()
            self.tbones_count: int = self._io.read_u4le()
            self.tbones_count_duplicate: int = self._io.read_u4le()
            self.unknown_array: int = self._io.read_u4le()
            self.bone_node_serial_numbers: list[int] = []
            for i in range(16):
                self.bone_node_serial_numbers.append(self._io.read_u2le())

            self.padding2: int = self._io.read_u2le()
    ...
    class TrimeshHeader(KaitaiStruct):
        ...
        def _read(self):
            ...
            self.texture_0_name = (KaitaiStream.bytes_terminate(self._io.read_bytes(32), 0, False)).decode("ASCII")
            self.texture_1_name = (KaitaiStream.bytes_terminate(self._io.read_bytes(32), 0, False)).decode("ASCII")
            self.texture_2_name = (KaitaiStream.bytes_terminate(self._io.read_bytes(12), 0, False)).decode("ASCII")
            self.texture_3_name = (KaitaiStream.bytes_terminate(self._io.read_bytes(12), 0, False)).decode("ASCII")
    ...
    def names_data(self):
        ...
        if self.model_header.name_offsets_count > 0:
            ...
            self._raw__m_names_data = self._io.read_bytes(
                ...
                - ((self.data_start + self.model_header.offset_to_name_offsets) + 4 * self.model_header.name_offsets_count)
            )
