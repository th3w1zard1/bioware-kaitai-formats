...
class Wav(KaitaiStruct):
    ...
    class Chunk(KaitaiStruct):
        ...
        def _read(self):
            self.id = (self._io.read_bytes(4)).decode("ASCII")
    ...
    class FormatChunkBody(KaitaiStruct):
        ...
        def _read(self):
            self.audio_format = KaitaiStream.resolve_enum(
                bioware_common.BiowareCommon.RiffWaveFormatTag, self._io.read_u2le()
            )
        ...
        def is_ima_adpcm(self):
            ...
            self._m_is_ima_adpcm = (
                self.audio_format
                == bioware_common.BiowareCommon.RiffWaveFormatTag.dvi_ima_adpcm
            )
        ...
        def is_mp3(self):
            ...
            self._m_is_mp3 = (
                self.audio_format
                == bioware_common.BiowareCommon.RiffWaveFormatTag.mpeg_layer3
            )
        ...
        def is_pcm(self):
            ...
            self._m_is_pcm = (
                self.audio_format == bioware_common.BiowareCommon.RiffWaveFormatTag.pcm
            )
    ...
    class RiffHeader(KaitaiStruct):
        ...
        def _read(self):
            self.riff_id = (self._io.read_bytes(4)).decode("ASCII")
            if not self.riff_id == "RIFF":
                raise kaitaistruct.ValidationNotEqualError(
                    "RIFF", self.riff_id, self._io, "/types/riff_header/seq/0"
                )
            self.riff_size = self._io.read_u4le()
            self.wave_id = (self._io.read_bytes(4)).decode("ASCII")
            if not self.wave_id == "WAVE":
                raise kaitaistruct.ValidationNotEqualError(
                    "WAVE", self.wave_id, self._io, "/types/riff_header/seq/2"
                )
