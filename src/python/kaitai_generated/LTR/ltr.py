...
class Ltr(KaitaiStruct):
    ...
    .. seealso::
       xoreos-docs — BioWare specs tree (no dedicated LTR Torlack page; use `ltrfile.cpp` + PyKotor) - https://github.com/xoreos/xoreos-docs/tree/master/specs/bioware
    """
    ...
    def _read(self):
        ...
        self.letter_count = KaitaiStream.resolve_enum(
            bioware_common.BiowareCommon.BiowareLtrAlphabetLength, self._io.read_u1()
        )
        self.single_letter_block = Ltr.LetterBlock(self._io, self, self._root)
        self.double_letter_blocks = Ltr.DoubleLetterBlocksArray(
            self._io, self, self._root
        )
        self.triple_letter_blocks = Ltr.TripleLetterBlocksArray(
            self._io, self, self._root
        )
