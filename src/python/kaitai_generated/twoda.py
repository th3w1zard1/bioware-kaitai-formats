...
class Twoda(KaitaiStruct):
    ...
    def _read(self):
        ...
        self._raw_cell_values_section = self._io.read_bytes(
            self.len_cell_values_section
        )
        _io__raw_cell_values_section = KaitaiStream(
            BytesIO(self._raw_cell_values_section)
        )
        self.cell_values_section = Twoda.CellValuesSection(
            _io__raw_cell_values_section, self, self._root
        )
    ...
    class CellValuesSection(KaitaiStruct):
        ...
        def _read(self):
            self.raw_data = (
                self._io.read_bytes(self._root.len_cell_values_section)
            ).decode("ASCII")
    ...
    class RowLabelEntry(KaitaiStruct):
        ...
        def _read(self):
            self.label_value = (self._io.read_bytes_term(9, False, True, False)).decode(
                "ASCII"
            )
    ...
    class TwodaHeader(KaitaiStruct):
        ...
        def is_valid_twoda(self):
            ...
            self._m_is_valid_twoda = (
                (self.magic == "2DA ")
                and (self.version == "V2.b")
                and (self.newline == 10)
            )
