...
type
  GffBtt* = ref object of KaitaiStruct
    `contents`*: Gff_GffUnionFile
    `parent`*: KaitaiStruct

proc read*(_: typedesc[GffBtt], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffBtt
...
proc read*(_: typedesc[GffBtt], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffBtt =
  ...
  this = new(GffBtt)
  let root = if root == nil: cast[GffBtt](this) else: cast[GffBtt](root)
  this.io = io
  this.root = root
  this.parent = parent
...
proc fromFile*(_: typedesc[GffBtt], filename: string): GffBtt =
  GffBtt.read(newKaitaiFileStream(filename), nil, nil)
