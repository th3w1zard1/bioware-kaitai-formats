...
type
  GffFac* = ref object of KaitaiStruct
    `contents`*: Gff_GffUnionFile
    `parent`*: KaitaiStruct

proc read*(_: typedesc[GffFac], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffFac
...
proc read*(_: typedesc[GffFac], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffFac =
  ...
  this = new(GffFac)
  let root = if root == nil: cast[GffFac](this) else: cast[GffFac](root)
  this.io = io
  this.root = root
  this.parent = parent
...
proc fromFile*(_: typedesc[GffFac], filename: string): GffFac =
  GffFac.read(newKaitaiFileStream(filename), nil, nil)
