...
type
  GffUti* = ref object of KaitaiStruct
    `contents`*: Gff_GffUnionFile
    `parent`*: KaitaiStruct

proc read*(_: typedesc[GffUti], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffUti
...
proc read*(_: typedesc[GffUti], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffUti =
  ...
  this = new(GffUti)
  let root = if root == nil: cast[GffUti](this) else: cast[GffUti](root)
  this.io = io
  this.root = root
  this.parent = parent
...
proc fromFile*(_: typedesc[GffUti], filename: string): GffUti =
  GffUti.read(newKaitaiFileStream(filename), nil, nil)
