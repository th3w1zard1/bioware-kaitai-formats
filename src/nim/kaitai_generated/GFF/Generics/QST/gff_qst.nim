...
type
  GffQst* = ref object of KaitaiStruct
    `contents`*: Gff_GffUnionFile
    `parent`*: KaitaiStruct

proc read*(_: typedesc[GffQst], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffQst
...
proc read*(_: typedesc[GffQst], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffQst =
  ...
  this = new(GffQst)
  let root = if root == nil: cast[GffQst](this) else: cast[GffQst](root)
  this.io = io
  this.root = root
  this.parent = parent
...
proc fromFile*(_: typedesc[GffQst], filename: string): GffQst =
  GffQst.read(newKaitaiFileStream(filename), nil, nil)
