...
type
  GffBte* = ref object of KaitaiStruct
    `contents`*: Gff_GffUnionFile
    `parent`*: KaitaiStruct

proc read*(_: typedesc[GffBte], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffBte
...
proc read*(_: typedesc[GffBte], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffBte =
  ...
  this = new(GffBte)
  let root = if root == nil: cast[GffBte](this) else: cast[GffBte](root)
  this.io = io
  this.root = root
  this.parent = parent
...
proc fromFile*(_: typedesc[GffBte], filename: string): GffBte =
  GffBte.read(newKaitaiFileStream(filename), nil, nil)
