...
type
  GffQdb* = ref object of KaitaiStruct
    `contents`*: Gff_GffUnionFile
    `parent`*: KaitaiStruct

proc read*(_: typedesc[GffQdb], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffQdb
...
proc read*(_: typedesc[GffQdb], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffQdb =
  ...
  this = new(GffQdb)
  let root = if root == nil: cast[GffQdb](this) else: cast[GffQdb](root)
  this.io = io
  this.root = root
  this.parent = parent
...
proc fromFile*(_: typedesc[GffQdb], filename: string): GffQdb =
  GffQdb.read(newKaitaiFileStream(filename), nil, nil)
