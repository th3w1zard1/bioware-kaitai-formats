...
type
  GffUtt* = ref object of KaitaiStruct
    `contents`*: Gff_GffUnionFile
    `parent`*: KaitaiStruct

proc read*(_: typedesc[GffUtt], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffUtt
...
proc read*(_: typedesc[GffUtt], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffUtt =
  ...
  this = new(GffUtt)
  let root = if root == nil: cast[GffUtt](this) else: cast[GffUtt](root)
  this.io = io
  this.root = root
  this.parent = parent
...
proc fromFile*(_: typedesc[GffUtt], filename: string): GffUtt =
  GffUtt.read(newKaitaiFileStream(filename), nil, nil)
