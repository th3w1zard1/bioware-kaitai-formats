...
type
  GffGic* = ref object of KaitaiStruct
    `contents`*: Gff_GffUnionFile
    `parent`*: KaitaiStruct

proc read*(_: typedesc[GffGic], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffGic
...
proc read*(_: typedesc[GffGic], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffGic =
  ...
  this = new(GffGic)
  let root = if root == nil: cast[GffGic](this) else: cast[GffGic](root)
  this.io = io
  this.root = root
  this.parent = parent
...
proc fromFile*(_: typedesc[GffGic], filename: string): GffGic =
  GffGic.read(newKaitaiFileStream(filename), nil, nil)
