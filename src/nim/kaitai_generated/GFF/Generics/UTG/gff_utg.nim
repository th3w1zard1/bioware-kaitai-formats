...
type
  GffUtg* = ref object of KaitaiStruct
    `contents`*: Gff_GffUnionFile
    `parent`*: KaitaiStruct

proc read*(_: typedesc[GffUtg], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffUtg
...
proc read*(_: typedesc[GffUtg], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffUtg =
  ...
  this = new(GffUtg)
  let root = if root == nil: cast[GffUtg](this) else: cast[GffUtg](root)
  this.io = io
  this.root = root
  this.parent = parent
...
proc fromFile*(_: typedesc[GffUtg], filename: string): GffUtg =
  GffUtg.read(newKaitaiFileStream(filename), nil, nil)
