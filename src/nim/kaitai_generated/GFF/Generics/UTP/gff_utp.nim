...
type
  GffUtp* = ref object of KaitaiStruct
    `contents`*: Gff_GffUnionFile
    `parent`*: KaitaiStruct

proc read*(_: typedesc[GffUtp], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffUtp
...
proc read*(_: typedesc[GffUtp], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffUtp =
  ...
  this = new(GffUtp)
  let root = if root == nil: cast[GffUtp](this) else: cast[GffUtp](root)
  this.io = io
  this.root = root
  this.parent = parent
...
proc fromFile*(_: typedesc[GffUtp], filename: string): GffUtp =
  GffUtp.read(newKaitaiFileStream(filename), nil, nil)
