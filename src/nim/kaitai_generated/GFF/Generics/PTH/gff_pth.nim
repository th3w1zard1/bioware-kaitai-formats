...
type
  GffPth* = ref object of KaitaiStruct
    `contents`*: Gff_GffUnionFile
    `parent`*: KaitaiStruct

proc read*(_: typedesc[GffPth], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffPth
...
proc read*(_: typedesc[GffPth], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffPth =
  ...
  this = new(GffPth)
  let root = if root == nil: cast[GffPth](this) else: cast[GffPth](root)
  this.io = io
  this.root = root
  this.parent = parent
...
proc fromFile*(_: typedesc[GffPth], filename: string): GffPth =
  GffPth.read(newKaitaiFileStream(filename), nil, nil)
