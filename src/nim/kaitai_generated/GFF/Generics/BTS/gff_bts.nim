...
type
  GffBts* = ref object of KaitaiStruct
    `contents`*: Gff_GffUnionFile
    `parent`*: KaitaiStruct

proc read*(_: typedesc[GffBts], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffBts
...
proc read*(_: typedesc[GffBts], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffBts =
  ...
  this = new(GffBts)
  let root = if root == nil: cast[GffBts](this) else: cast[GffBts](root)
  this.io = io
  this.root = root
  this.parent = parent
...
proc fromFile*(_: typedesc[GffBts], filename: string): GffBts =
  GffBts.read(newKaitaiFileStream(filename), nil, nil)
