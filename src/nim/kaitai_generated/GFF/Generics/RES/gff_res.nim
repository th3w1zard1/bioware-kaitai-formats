...
type
  GffRes* = ref object of KaitaiStruct
    `contents`*: Gff_GffUnionFile
    `parent`*: KaitaiStruct

proc read*(_: typedesc[GffRes], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffRes
...
proc read*(_: typedesc[GffRes], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffRes =
  ...
  this = new(GffRes)
  let root = if root == nil: cast[GffRes](this) else: cast[GffRes](root)
  this.io = io
  this.root = root
  this.parent = parent
...
proc fromFile*(_: typedesc[GffRes], filename: string): GffRes =
  GffRes.read(newKaitaiFileStream(filename), nil, nil)
