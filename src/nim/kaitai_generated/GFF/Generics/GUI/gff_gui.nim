...
type
  GffGui* = ref object of KaitaiStruct
    `contents`*: Gff_GffUnionFile
    `parent`*: KaitaiStruct

proc read*(_: typedesc[GffGui], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffGui
...
proc read*(_: typedesc[GffGui], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffGui =
  ...
  this = new(GffGui)
  let root = if root == nil: cast[GffGui](this) else: cast[GffGui](root)
  this.io = io
  this.root = root
  this.parent = parent
...
proc fromFile*(_: typedesc[GffGui], filename: string): GffGui =
  GffGui.read(newKaitaiFileStream(filename), nil, nil)
