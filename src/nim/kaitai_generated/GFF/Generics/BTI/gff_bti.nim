...
type
  GffBti* = ref object of KaitaiStruct
    `contents`*: Gff_GffUnionFile
    `parent`*: KaitaiStruct

proc read*(_: typedesc[GffBti], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffBti
...
proc read*(_: typedesc[GffBti], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffBti =
  ...
  this = new(GffBti)
  let root = if root == nil: cast[GffBti](this) else: cast[GffBti](root)
  this.io = io
  this.root = root
  this.parent = parent
...
proc fromFile*(_: typedesc[GffBti], filename: string): GffBti =
  GffBti.read(newKaitaiFileStream(filename), nil, nil)
