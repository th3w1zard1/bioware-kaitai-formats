...
type
  GffUtc* = ref object of KaitaiStruct
    `contents`*: Gff_GffUnionFile
    `parent`*: KaitaiStruct

proc read*(_: typedesc[GffUtc], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffUtc
...
proc read*(_: typedesc[GffUtc], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): GffUtc =
  ...
  this = new(GffUtc)
  let root = if root == nil: cast[GffUtc](this) else: cast[GffUtc](root)
  this.io = io
  this.root = root
  this.parent = parent
...
proc fromFile*(_: typedesc[GffUtc], filename: string): GffUtc =
  GffUtc.read(newKaitaiFileStream(filename), nil, nil)
