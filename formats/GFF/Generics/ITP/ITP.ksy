meta:
  id: gff_itp
  title: BioWare GFF3 ITP (ITP template)
  license: MIT
  endian: le
  file-extension: itp
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2030 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per ITP template docs / PyKotor `gff_auto`.
