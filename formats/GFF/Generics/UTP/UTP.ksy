meta:
  id: gff_utp
  title: BioWare GFF3 UTP (UTP template)
  license: MIT
  endian: le
  file-extension: utp
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2044 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per UTP template docs / PyKotor `gff_auto`.
