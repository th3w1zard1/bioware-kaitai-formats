meta:
  id: gff_bts
  title: BioWare GFF3 BTS (BTS template)
  license: MIT
  endian: le
  file-extension: bts
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2034 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per BTS template docs / PyKotor `gff_auto`.
