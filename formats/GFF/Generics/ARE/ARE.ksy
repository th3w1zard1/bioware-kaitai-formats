meta:
  id: gff_are
  title: BioWare GFF3 ARE (ARE template)
  license: MIT
  endian: le
  file-extension: are
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2012 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per ARE template docs / PyKotor `gff_auto`.
