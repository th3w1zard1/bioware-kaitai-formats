meta:
  id: gff_bte
  title: BioWare GFF3 BTE (BTE template)
  license: MIT
  endian: le
  file-extension: bte
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2039 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per BTE template docs / PyKotor `gff_auto`.
