meta:
  id: gff_utd
  title: BioWare GFF3 UTD (UTD template)
  license: MIT
  endian: le
  file-extension: utd
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2042 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per UTD template docs / PyKotor `gff_auto`.
