meta:
  id: gff_uts
  title: BioWare GFF3 UTS (UTS template)
  license: MIT
  endian: le
  file-extension: uts
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2035 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per UTS template docs / PyKotor `gff_auto`.
