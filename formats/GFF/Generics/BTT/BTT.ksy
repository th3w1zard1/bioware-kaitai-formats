meta:
  id: gff_btt
  title: BioWare GFF3 BTT (BTT template)
  license: MIT
  endian: le
  file-extension: btt
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2031 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per BTT template docs / PyKotor `gff_auto`.
