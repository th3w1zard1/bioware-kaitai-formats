meta:
  id: gff_res
  title: BioWare GFF3 RES (RES template)
  license: MIT
  endian: le
  file-extension: res
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 0 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per RES template docs / PyKotor `gff_auto`.
