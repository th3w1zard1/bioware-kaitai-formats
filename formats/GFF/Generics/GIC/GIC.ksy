meta:
  id: gff_gic
  title: BioWare GFF3 GIC (GIC template)
  license: MIT
  endian: le
  file-extension: gic
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2046 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per GIC template docs / PyKotor `gff_auto`.
