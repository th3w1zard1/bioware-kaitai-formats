meta:
  id: gff_dlg
  title: BioWare GFF3 DLG (DLG template)
  license: MIT
  endian: le
  file-extension: dlg
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2029 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per DLG template docs / PyKotor `gff_auto`.
