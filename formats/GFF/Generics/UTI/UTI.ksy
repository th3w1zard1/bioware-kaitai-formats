meta:
  id: gff_uti
  title: BioWare GFF3 UTI (UTI template)
  license: MIT
  endian: le
  file-extension: uti
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2025 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per UTI template docs / PyKotor `gff_auto`.
