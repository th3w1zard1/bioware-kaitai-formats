meta:
  id: gff_bic
  title: BioWare GFF3 BIC (BIC template)
  license: MIT
  endian: le
  file-extension: bic
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2015 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per BIC template docs / PyKotor `gff_auto`.
