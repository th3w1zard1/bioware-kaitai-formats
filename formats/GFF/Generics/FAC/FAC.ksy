meta:
  id: gff_fac
  title: BioWare GFF3 FAC (FAC template)
  license: MIT
  endian: le
  file-extension: fac
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2038 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per FAC template docs / PyKotor `gff_auto`.
