meta:
  id: gff_cam
  title: BioWare GFF3 CAM (CAM template)
  license: MIT
  endian: le
  file-extension: cam
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2104 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per CAM template docs / PyKotor `gff_auto`.
