meta:
  id: gff_pth
  title: BioWare GFF3 PTH (PTH template)
  license: MIT
  endian: le
  file-extension: pth
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 3003 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per PTH template docs / PyKotor `gff_auto`.
