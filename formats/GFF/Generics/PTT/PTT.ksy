meta:
  id: gff_ptt
  title: BioWare GFF3 PTT (PTT template)
  license: MIT
  endian: le
  file-extension: ptt
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2066 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per PTT template docs / PyKotor `gff_auto`.
