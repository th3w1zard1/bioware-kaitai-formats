meta:
  id: gff_btg
  title: BioWare GFF3 BTG (BTG template)
  license: MIT
  endian: le
  file-extension: btg
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2054 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per BTG template docs / PyKotor `gff_auto`.
