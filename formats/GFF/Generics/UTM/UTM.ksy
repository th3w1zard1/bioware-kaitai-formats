meta:
  id: gff_utm
  title: BioWare GFF3 UTM (UTM template)
  license: MIT
  endian: le
  file-extension: utm
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2051 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per UTM template docs / PyKotor `gff_auto`.
