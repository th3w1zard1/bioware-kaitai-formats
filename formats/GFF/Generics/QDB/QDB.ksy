meta:
  id: gff_qdb
  title: BioWare GFF3 QDB (QDB template)
  license: MIT
  endian: le
  file-extension: qdb
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2083 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per QDB template docs / PyKotor `gff_auto`.
