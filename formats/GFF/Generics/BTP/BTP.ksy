meta:
  id: gff_btp
  title: BioWare GFF3 BTP (BTP template)
  license: MIT
  endian: le
  file-extension: btp
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2043 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per BTP template docs / PyKotor `gff_auto`.
