meta:
  id: gff_ute
  title: BioWare GFF3 UTE (UTE template)
  license: MIT
  endian: le
  file-extension: ute
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2040 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per UTE template docs / PyKotor `gff_auto`.
