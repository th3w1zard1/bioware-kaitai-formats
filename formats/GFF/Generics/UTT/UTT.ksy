meta:
  id: gff_utt
  title: BioWare GFF3 UTT (UTT template)
  license: MIT
  endian: le
  file-extension: utt
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2032 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per UTT template docs / PyKotor `gff_auto`.
