meta:
  id: gff_qst
  title: BioWare GFF3 QST (QST template)
  license: MIT
  endian: le
  file-extension: qst
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2084 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per QST template docs / PyKotor `gff_auto`.
