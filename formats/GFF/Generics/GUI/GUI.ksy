meta:
  id: gff_gui
  title: BioWare GFF3 GUI (GUI template)
  license: MIT
  endian: le
  file-extension: gui
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2047 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per GUI template docs / PyKotor `gff_auto`.
