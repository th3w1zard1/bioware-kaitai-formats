meta:
  id: gff_ifo
  title: BioWare GFF3 IFO (IFO template)
  license: MIT
  endian: le
  file-extension: ifo
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2014 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per IFO template docs / PyKotor `gff_auto`.
