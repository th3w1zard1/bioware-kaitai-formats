meta:
  id: gff_ptm
  title: BioWare GFF3 PTM (PTM template)
  license: MIT
  endian: le
  file-extension: ptm
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2065 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per PTM template docs / PyKotor `gff_auto`.
