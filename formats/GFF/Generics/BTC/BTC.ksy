meta:
  id: gff_btc
  title: BioWare GFF3 BTC (BTC template)
  license: MIT
  endian: le
  file-extension: btc
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2026 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per BTC template docs / PyKotor `gff_auto`.
