meta:
  id: gff_bti
  title: BioWare GFF3 BTI (BTI template)
  license: MIT
  endian: le
  file-extension: bti
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2024 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per BTI template docs / PyKotor `gff_auto`.
