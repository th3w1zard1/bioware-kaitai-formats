meta:
  id: gff_sav
  title: BioWare GFF3 SAV (SAV template)
  license: MIT
  endian: le
  file-extension: sav
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2057 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per SAV template docs / PyKotor `gff_auto`.
