meta:
  id: gff_jrl
  title: BioWare GFF3 JRL (JRL template)
  license: MIT
  endian: le
  file-extension: jrl
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2056 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per JRL template docs / PyKotor `gff_auto`.
