meta:
  id: gff_utg
  title: BioWare GFF3 UTG (UTG template)
  license: MIT
  endian: le
  file-extension: utg
  imports:
  ...
  xref:
    ...
    xoreos_file_type_id_value: |
      Numeric id 2055 (`xoreos_file_type_id` in `formats/Common/bioware_type_ids.ksy`).
...
seq:
  - id: contents
    ...
    doc: Full GFF3/GFF4 union (see `GFF.ksy`); interpret struct labels per UTG template docs / PyKotor `gff_auto`.
